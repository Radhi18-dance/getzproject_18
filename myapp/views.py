...
def custom_login(request):
    ...
    if request.method == 'POST':
        ...
        user = login_tbl.objects.filter(email=email, password=password).first()
...
def profile(request):
    ...
    if not cuser:
        return redirect('/')  # Or redirect to 'login' if that's what you intend
    ...
    return render(request, 'profile.html', {'cuser': cuser, 'data': data})
...
def deleteprofile(request,id):
    stid=login_tbl.objects.get(id=id)
    login_tbl.delete(stid)
    return redirect('profile')
...
def agent_commission_report(request):
    ...
    date_wise_data = queryset.values('entry_date').order_by('entry_date').annotate(
        total_commission=Sum('agency_comm_amt'),
        total_premium=Sum('total_premium')
    )

    total_commission = queryset.aggregate(Sum('agency_comm_amt'))['agency_comm_amt__sum'] or 0
    total_premium = queryset.aggregate(Sum('total_premium'))['total_premium__sum'] or 0
